...
sub gt_ok ($$) {
    my ($rank1, $rank2) = @_;
    ok(Games::Go::Rank->new(rank => $rank1) > Games::Go::Rank->new(rank => $rank2),
        "both OO: $rank1 > $rank2");
...
sub value_ok ($$) {
    my ($rank, $value) = @_;
    is(Games::Go::Rank->new(rank => $rank)->as_value, $value,
        "value of $rank is $value");
    is(Games::Go::Rank->new->from_value($value)->rank, $rank,
        "rank of value $value is $rank");
}
